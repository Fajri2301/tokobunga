...
<div class="max-w-2xl mx-auto">
    <div class="flex items-center gap-4 mb-6">
        ...
        <div>
            <h2 class="text-xl font-black text-[#545454] tracking-tight">Edit Profil Admin</h2>
            <p class="text-sm text-slate-400 font-medium">Ubah nama, email, atau reset password.</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-3xl shadow-xl shadow-slate-200/40 border border-slate-100 overflow-hidden">
        <form action="{{ route('admin.users.update', $user) }}" method="POST" class="p-6 md:p-8 space-y-6">
            ...
            <div class="pt-6 mt-6 border-t border-slate-100">
                <h3 class="text-sm font-black text-[#545454] uppercase tracking-widest mb-4">Ubah Password <span class="text-slate-400 text-xs normal-case tracking-normal">(Opsional)</span></h3>
                <p class="text-xs text-slate-500 mb-6">Kosongkan kolom password di bawah ini jika Anda tidak ingin mengubah password akun ini.</p>
                
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <!-- Password -->
                    <div>
                        <label for="password" class="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Password Baru</label>
                    ...
                    <!-- Confirm Password -->
                    <div>
                        <label for="password_confirmation" class="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Konfirmasi Password Baru</label>
            ...
            <div class="pt-6 mt-6 border-t border-slate-100 flex justify-between items-center">
                @if(auth()->id() === $user->id)
                    <span class="text-xs font-bold text-slate-400 uppercase tracking-widest">Ini adalah akun Anda</span>
                @else
                    <div></div>
                @endif
