...
<div class="max-w-4xl">
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-8 border-b border-gray-50 flex items-center">
            ...
            <h2 class="text-xl font-bold text-gray-800">Edit Kategori: {{ $category->name }}</h2>
        </div>
        
        <form action="{{ route('admin.categories.update', $category->id) }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-6">
            @csrf
            @method('PUT')
            <div>
                <label for="name" class="block text-sm font-semibold text-gray-700 mb-2">Nama Kategori</label>
                <input type="text" name="name" id="name" value="{{ $category->name }}" class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition" required>
            </div>

            <div class="flex items-center">
                <input type="checkbox" name="is_active_on_home" id="is_active_on_home" value="1" {{ $category->is_active_on_home ? 'checked' : '' }} class="h-5 w-5 text-blue-600 border-gray-300 rounded focus:ring-blue-500 transition">
            ...
            <div>
                ...
                <div class="mt-4">
                    ...
                    <img src="{{ asset('storage/' . $category->image) }}" class="h-32 rounded-lg border border-gray-100">
                </div>
                @endif
            </div>
